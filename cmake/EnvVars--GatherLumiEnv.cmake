...
function(lumi_collect_env)
  ...
  foreach(PFX IN LISTS _prefixes)
    ...
    set(_root "")
    set(_root_from_default "")

    list(LENGTH _defaults _defaults_len)
    if(_defaults_len GREATER 1)
      math(EXPR _defaults_last "${_defaults_len} - 1")
      foreach(_didx RANGE 0 ${_defaults_last} 2)
        math(EXPR _dnext "${_didx} + 1")
        if(_dnext LESS _defaults_len)
          list(GET _defaults ${_didx} _n)
          list(GET _defaults ${_dnext} _p)
          if("${_n}" STREQUAL "${PFX}")
            _lumi_norm_path(_root "${_p}")
            set(_root_from_default "${_root}")
            break()
          endif()
        endif()
      endforeach()
    ...
    if(NOT _root AND DEFINED ENV{${_rootvar}} AND NOT "$ENV{${_rootvar}}" STREQUAL "")
      _lumi_norm_path(_root "$ENV{${_rootvar}}")
    endif()

    if(NOT _root)
      _lumi_norm_path(_root "${CMAKE_SOURCE_DIR}/deps/${PFX}")
    elseif(_root_from_default AND DEFINED ENV{${_rootvar}} AND NOT "$ENV{${_rootvar}}" STREQUAL "")
      _lumi_norm_path(_env_root "$ENV{${_rootvar}}")
      if(NOT _env_root STREQUAL _root_from_default)
        message(STATUS "[Lumi][Env] Ignoring ${_rootvar} from environment ('${_env_root}') in favor of preset default '${_root_from_default}'")
